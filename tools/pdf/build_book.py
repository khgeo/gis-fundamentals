...
TITLE = "មូលដ្ឋានគ្រឹះនៃប្រព័ន្ធព័ត៌មានភូមិសាស្ត្រ"
PORT = 8765
KM = "០១២៣៤៥៦៧៨៩"
...
<p>លំហាត់ទាំងអស់ប្រើសំណុំទិន្នន័យ Cambodia (៤៣ ស្រទាប់) ដែលពិពណ៌នាក្នុងឧបសម្ព័ន្ធ ខ។ មាត្រដ្ឋាន ចំណោល និងការរចនាផែនទី ជាខ្លឹមសារសៀវភៅទី១ ឯរូបវិទ្យាវិសាលគម និងការចាត់ថ្នាក់រូបភាព ជាខ្លឹមសារសៀវភៅទី៣។</p>
<p><b>ពិសោធន៍ និងផែនទីអន្តរកម្ម</b>៖ កំណែអនឡាញមានផែនទី ពិសោធន៍ ល្បែងផ្គូផ្គង និងប្រអប់ពិនិត្យលទ្ធផលដោយខ្លួនឯង។ ក្នុងសៀវភៅបោះពុម្ពនេះ ពួកវាបង្ហាញតែស្ថានភាពដំបូង ជាមួយកូដ QR ដែលនាំទៅទំព័រអនឡាញ។</p></div>
...
async def render(chrome_path=None):
    ...
    doc = f'<!doctype html><html lang="km"><head><meta charset="utf-8">{head}<style>{PRINT_CSS}</style></head><body data-md-color-scheme="default" data-md-color-primary="teal" data-md-color-accent="deep-orange"><div class="md-typeset">{"".join(body)}</div><script src="{leaflet_js}"></script>{scripts}</body></html>'
    ...
    header = f'<div style="width:100%;font-family:Battambang;font-size:7.5pt;color:#888;padding:0 18mm 0 20mm;display:flex;justify-content:space-between"><span>{TITLE}</span><span>សៀវភៅទី២</span></div>'
